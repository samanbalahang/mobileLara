...
<link href="{{url('/')}}/dashboard/assets/css/swiper-bundle.min.css" rel="stylesheet">
...
<div class="col-12 col-md-10">
    <main>
        ...
        @include('dashboard.Templats.settings.image')
        @include('dashboard.Templats.settings.slider')

        <!-- <div class="swiper-container mySwiperResponsive theCarosel">
...
<script src="{{url('/')}}/dashboard/assets/js/swiper-bundle.min.js"></script>
<script>
    ...
    function templateFatherClasses() {
        ...
        $("#templateFather #textEditor").removeClass("disabled");
        ...
        $("#templateFather #icon").removeClass("disabled");
        $("#templateFather #accordion").removeClass("disabled");
        $("#templateFather #gallery").removeClass("disabled");


        /*
            المان تمپلیت متن را حذف غیر فعال کرده ایم
            در آینده ممکن است فعال شود 
        */
    ...
    */
    ...
    // روی ایکون عکس کلیک شد.
    function imageClicked(fatherId){
        $("#settingsImg").attr("fatherId",fatherId);
        $("#settingsImg").show();
    }

    // اگر روی عکس های داخل مدال تنظیمات عکس کلیک شد.
    $("#medias-here img").on("click",function(e){
        fatherId = $("#settingsImg").attr("fatherId");
        src = $(this).attr("src");
        alt = $(this).attr("alt");
        let classes = "img img-elemant";
        let imgElemant = document.createElement("img");
        $(imgElemant).attr("src",src);
        $(imgElemant).attr("alt",alt);
        $(imgElemant).addClass("w-100");
        let parent = document.getElementById(fatherId);
        $(parent).empty();
        $(imgElemant).addClass("img img-elemant");
        let randumID = randumId();
        imgElemant.id = randumID;
        $(parent).append(imgElemant);
        // درج html
        insertinHtml("img",randumID,fatherId,classes,'{"src":"'+src+'","alt":"'+alt+'"}',"noclose");
        $("#settingsImg").hide();
        return randumID;
    });

    // روی جداکننده کلیک شد.
    function sepratorClicked(fatherId){
        let hrElemantD = document.createElement("div");
        let hrElemant = document.createElement("hr");
        let parent = document.getElementById(fatherId);
        let classes = "hr hr-elemant";
        $(parent).empty();

        $(hrElemant).addClass(classes);
        $(hrElemantD).append($(hrElemant));
        let randumIDD = randumId();
        hrElemantD.id = randumIDD;
        randumID = randumId();
        hrElemant.id = randumID;
        $(parent).append(hrElemantD);
        // درج html
        insertinHtml("div",randumIDD,fatherId,classes);
        insertinHtml("hr",randumID,randumIDD,classes,"","noclose");
    }

    // روی اسلایدر کلیک شد.
    function sliderClicked(fatherId){
        $("#settingsSlider").attr("fatherId",fatherId);
        $("#settingsSlider").show();
    }

    // در تنظیمات اسلایدر نوع اسلایدر را تغییر دهیم امکان تعیین دسته بندی ظاهر میشود.
    $("#whatGetSlide").on("change",function(e){
        if($(this).val()!=0){
            $("#selectSliderCat").show();
        }else{
            $("#selectSliderCat").hide();
        }
    });

    // تنظیمات اسلایدر کلیک شده است.
    $("#sliders-style-tab-pane-apply").on("click",function(e){
        let type = $("#type").val();
        let hasDot = $("#hasDot").val();
        let hasNextPrev = $("#hasNextPrev").val();
        let whatGetSlide = $("#whatGetSlide").val();
        if($(whatGetSlide) != 0){
            let SelectCatSliders = $("#SelectCatSliders").val();
        }
        let slidersCount =  $("#hasDot").val();
        if(whatGetSlide ==0){
            runTestSlider();
        }
        $("#settingsSlider").hide();
        e.preventDefault();
         ...
         // تنظیمات اسلایدر ها
    var theSlider = new Swiper(".theSlider", {
        slidesPerView: 1,
        spaceBetween: 10,
        lazy: true,
        init:false,
    ...
    var theCarosel = new Swiper(".theCarosel", {
        slidesPerView: 1,
        spaceBetween: 10,
        init:false,
        lazy: true,
    ...
    // کاربر تقاضای راه اندازی اسلایدر تستی را داده است.
   
    function runTestSlider(){
        let fatherId =  $("#settingsSlider").attr("fatherId");
        let html ='<div class="swiper-container mySwiperResponsive theCarosel">'+'<div class="swiper-wrapper">'
        +'<div class="swiper-slide">Slide 1</div>'
        +'<div class="swiper-slide">Slide 2</div>'
        +'<div class="swiper-slide">Slide 3</div>'
        +'<div class="swiper-slide">Slide 4</div>'
        ...
        +'</div>';
        let randumID = randumId();
        let sliderElemant = createElementFromHTML(html,randumID);
        let parent = document.getElementById(fatherId);
        $(parent).empty();
        // $(containerElemant).addClass("container container-elemant");
        // $(containerElemant).append($(".fa-plus").first().clone());
        $(parent).append(sliderElemant);
        // theSlider.init();
        theCarosel.init();
        // درج html
        insertinHtml("container",randumID,fatherId);
        insertinHtml("div",randumID,fatherId,classes="swiper-container mySwiperResponsive theCarosel",attr="",type="close");
        insertinHtml("div",0,randumID,classes="swiper-wrapper","","close","Slide 1");
        insertinHtml("div",0,randumID,classes="swiper-wrapper","","close","Slide 2");
        insertinHtml("div",0,randumID,classes="swiper-wrapper","","close","Slide 3");
        insertinHtml("div",0,randumID,classes="swiper-wrapper","","close","Slide 4");
        insertinHtml("div",0,randumID,classes="swiper-wrapper","","close","Slide 5");
        insertinHtml("div",0,randumID,classes="swiper-wrapper","","close","Slide 6");
        insertinHtml("div",0,randumID,classes="swiper-wrapper","","close","Slide 7");
        insertinHtml("div",0,randumID,classes="swiper-wrapper","","close","Slide 8");
        insertinHtml("div",0,randumID,classes="swiper-wrapper","","close","Slide 9");
         // // $("#html").val("<row>");
        // $(containerElemant).append($(".close-ctx").clone());
        // $(containerElemant).append($(".ctxMenu").clone());
        // $("#" + randumID + " .close-ctx").on("click", function(e) {
    ...
    // روی فرم کلیک شده است
    function formsClicked(fatherId){
        $("#settingsForm").attr("fatherId",fatherId);
        $("#settingsForm").show();                                  
    }

    // ایجاد عنصر با یک رشته متنی 
    function createElementFromHTML(htmlString,randumID) {
        var div = document.createElement('div');
        div.innerHTML = htmlString.trim();
        let sliderElemant =  div.firstChild;
        sliderElemant.id = randumID;
    ...
    // create html
    function insertinHtml(ElemantName,randumID,fatherId,classes=" ",attr="",type="close",node=""){
        ...
        if (ElemantName == "row" || ElemantName == "container" || ElemantName == "container-fluid" ){
            ...
            if (attr !="") {
                ...
                for (const [key, value] of Object.entries(attr)) {
                    attributsTxt =attributsTxt+" "+key+" = '"+value+"'";
                }          
            }
            if(type == "close"){
                if (node =="") {
                    elamentText  = "<"+ElemantName+"  class='"+classes+"' "+attributsTxt +" id='"+randumID+"'></"+ElemantName+">";
                }else{
                    elamentText  = "<"+ElemantName+"  class='"+classes+"' "+attributsTxt +" id='"+randumID+"'>"+node+"</"+ElemantName+">";

                }
            }
            else{
                elamentText  = "<"+ElemantName+"  class='"+classes+"' "+attributsTxt+"  "+" id='"+randumID+"'>";
            }
        }
    ...
    /* عنصر ما برای ایجاد شدن والد میخواهد این والد بصورت پیشفرض
    Container,
    Row
    هستند
    */
   ...
   function addEventListeners(addnewrow,columnNember,fatherId,classes,parent,lastColID){
    ...
    /* 
    دسته بندی های اسلایدر در صفحه تنظیمات اسلایدر پیشفرض مخفی است 
    و با انتخاب موضوع اسلایدر از بکند پر میشوند.
    */
    $("#selectSliderCat").hide();
